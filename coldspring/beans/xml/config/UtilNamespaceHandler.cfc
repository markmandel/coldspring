...
<cffunction name="getSchemaLocations" hint="to give ColdSpring a map of remote Schemas to their local absolute paths"
	...
	<cfscript>
		var map = {};
		var path = getDirectoryFromPath(getMetadata(this).path) & "/coldspring-util-2.0.xsd";

		map["http://coldspringframework.org/schema/coldspring-util-2.0.xsd"] = path;
		map["http://www.coldspringframework.org/schema/coldspring-util-2.0.xsd"] = path;
